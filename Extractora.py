...
class BagFileProcessor:
    ...
    def save_depth_image(self, depth_frame):
        ...
        cv2.imwrite('ArchivosDeLaExtraccion/Depth/depth_image.png', depth_colormap)

    def save_color_image(self, aligned_frames):
        color_frame = aligned_frames.get_color_frame()
        color_image = np.asanyarray(color_frame.get_data())
        cv2.imwrite('ArchivosDeLaExtraccion/RGBcolor_image.png', color_image)

    def save_pointcloud(self, depth_frame, aligned_frames):
        pc = rs.pointcloud()
        pc.map_to(aligned_frames.get_color_frame())
        points = pc.calculate(depth_frame)
        points.export_to_ply('ArchivosDeLaExtraccion/Ply/output.ply', aligned_frames.get_color_frame())
